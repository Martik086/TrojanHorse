...
const BotCard = React.memo(({ bot, isSelected, onSelect, onHover }: { bot: Bot, isSelected: boolean, onSelect: () => void, onHover: () => void }) => (
  <motion.div
    className={`rounded-lg p-2 mr-1 ml-1 cursor-pointer transition-all ${
      isSelected 
        ...
        : 'bg-transparent hover:bg-zinc-750 border border-zinc-700'
    }`}
    onClick={onSelect}
  ...
  >
    <div className="flex items-center">
      ...
      <h4 className="text-base font-medium text-zinc-100 ml-2 truncate max-w-[calc(100%-2.5rem)]">{bot.name}</h4>
    </div>
  </motion.div>
))
  ...
  return (
      <ErrorBoundary FallbackComponent={ErrorFallback}>
        <div className="min-h-screen w-full bg-zinc-950 flex items-center justify-center p-4">
          <div className="w-full max-w-7xl bg-zinc-950 rounded-lg shadow-lg p-6 border border-zinc-800 flex items-stretch">
            {/* User Profile Section */}
            <div className="w-1/2 pr-4 flex items-center justify-center border-r border-zinc-800">
              <div className="max-w-sm w-full">
                <h2 className="text-3xl font-bold text-zinc-100 mb-6 text-center">Quick Play</h2>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Select value={language} onValueChange={setLanguage}>
                      <SelectTrigger className="w-full text-lg bg-zinc-800 text-zinc-100 border-zinc-700 focus:border-zinc-500">
            ...
            <div className="w-1/2 pl-4 flex flex-col">
              <h2 className="text-3xl font-bold text-zinc-100 mb-4">Select Bots</h2>

              {/* Bot Info Section */}
              <div className={`rounded-lg p-4 mb-4 h-40 overflow-y-auto ${
                  hoveredBot
                ...
                {hoveredBot ? (
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        <h3 className="text-xl font-semibold text-zinc-100">{hoveredBot.name}</h3>
                        <Badge
                            variant="secondary"
                            className={`bg-zinc-900 text-base ${difficultyStyles[hoveredBot.difficulty as keyof typeof difficultyStyles].text}`}
                        >
              ...
              <ScrollArea className="flex-grow pr-4">
                {difficultyOrder.map(difficulty => (
                    <div key={difficulty} className="mb-4">
                      <h3 className="text-xs font-bold text-zinc-300 font-mono mb-2 uppercase">{difficulty}</h3>
